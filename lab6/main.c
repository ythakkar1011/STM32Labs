...
void setup_adc(void)
{
    ...
    GPIOA->MODER |= 0xf; //Set pins 1,0 for analog input
    RCC->APB2ENR |= RCC_APB2ENR_ADC1EN; //Enable clock to ADC unit
    RCC->CR2 |= RCC_CR2_HSI14ON; //Turn on clock
    while(!(RCC->CR2 & RCC_CR2_HSI14RDY)); //wait for clock
    ADC1->CR |= ADC_CR_ADEN; //enable ADC
    while(!(ADC1->ISR & ADC_ISR_ADRDY)); //wait for ADC
}
...
int read_adc(void)
{
    ...
    return /* replace with something meaningful */ ADC1 -> DR;
}
